# --- START OF FILE master_xmlrpc.py (Modified for Docker with Clear Output) ---
...
def run_pipeline(data_size):
    print("=" * 60)
    print("🚀 DOCKER XML-RPC MASTER ACTIVITY 🚀")
    print("=" * 60)
...
# Listen on all available network interfaces for Docker
server = SimpleXMLRPCServer(("0.0.0.0", 8005), allow_none=True)
server.register_function(run_pipeline, "run_pipeline")
print("MasterService (XML-RPC) running on port 8005")
server.serve_forever()
# --- END OF FILE master_xmlrpc.py (Modified for Docker with Clear Output) ---